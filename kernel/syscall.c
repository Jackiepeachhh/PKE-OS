 ...
 */
 ...
 ssize_t sys_user_semnew(int init_value)
 {
   if(init_value < 0) panic("illegal init semphore value!\n");
   else return do_semnew(init_value);
 }
 
 ssize_t sys_user_semP(int sem_index)
 {
   do_semP(sem_index);
   return 0;
 }
 
 ssize_t sys_user_semV(int sem_index)
 {
   do_semV(sem_index);
   return 0;
 }
