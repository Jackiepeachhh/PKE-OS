...
static void *elf_alloc_mb(elf_ctx *ctx, uint64 elf_pa, uint64 elf_va, uint64 size) {
  ...
  user_vm_map((pagetable_t)msg->p->pagetable, elf_va, PGSIZE, (uint64)pa, prot_to_type(PROT_WRITE | PROT_READ | PROT_EXEC, 1));
...
static void *elf_alloc_mb_vfs(elf_ctx *ctx, uint64 elf_pa, uint64 elf_va, uint64 size) {
  ...
  memset((void *)pa, 0, PGSIZE);
  pagetable_t pte = page_walk((pagetable_t)msg->p->pagetable, elf_va, 0);
  if(pte != 0 && *pte & PTE_V)
  {
