 ...
 */
 ...
 elf_status elf_load(elf_ctx *ctx) {
   ...
   // load the strtab header
   uint64 strtab_h_off = ctx->ehdr.shoff + ctx->ehdr.shstrndx * sizeof(elf_sect_header);
   elf_sect_header strtab_h;
   elf_fpread(ctx, (void *)&strtab_h, sizeof(strtab_h), strtab_h_off);
 
   // load the strtab
   char strtab[strtab_h.size];
   elf_fpread(ctx, &strtab, strtab_h.size, strtab_h.offset);
 
   // read the header table， find the debug_line section header
   elf_sect_header debug_line_h;
   for(int i = 0; i < ctx->ehdr.shnum; i ++)
   {
     elf_fpread(ctx, (void *)&debug_line_h, sizeof(elf_sect_header), ctx->ehdr.shoff + i * ctx->ehdr.shentsize);
     if(strcmp(".debug_line", strtab + debug_line_h.name) == 0) break;
   }
 
   // read the debug line raw data and feed it to the make_addr_line function
   elf_fpread(ctx, (void *)debug_line_raw, debug_line_h.size, debug_line_h.offset);
   make_addr_line(ctx, debug_line_raw, debug_line_h.size);
