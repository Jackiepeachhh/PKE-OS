 ...
 */
 ...
 int signal[MAX_signal_num], st[MAX_signal_num], proc_index[MAX_signal_num][MAX_signal_num];
 ...
 {
   if(init_value < 0) panic("illegal init_value!\n");
   int i;
   for(i = 0; i < MAX_signal_num; i ++) 
     if(!st[i]){
       st[i] = 1;  // in use
       signal[i] = init_value;
       for(int j = 0; j < MAX_signal_num; j ++) proc_index[i][j] = -1; // there is no proc waiting for this semaphore
       break;
     }
 ...
 {
   if(!st[sem_index]) panic("illegal sem_index!\n");
 
   if(signal[sem_index] <= 0){
     // proc should wait for semaphore
 
     for(int j = 0; j < MAX_signal_num; j ++)
     {
       if(proc_index[sem_index][j] != -1) continue;
       proc_index[sem_index][j] = current->pid;  // record the proc 
       break;
     }
     current->status = BLOCKED;
 
 
     // sprint("\n\n!! proc %d wait for  %d , sem now is %d !!\n\n", current->pid, sem_index, signal[sem_index] - 1);
   ...
   {
     signal[sem_index] --;
     // sprint("\n\n!! proc %d semP %d , sem now is %d !!\n\n", current->pid, sem_index, signal[sem_index]);
   }
 }
 ...
 void do_semV(int sem_index)
 {
   ...
   if(signal[sem_index] <= -1){ // should wakeup one proc
     
     int block_pid = -1;
     for(int j = 0; j < MAX_signal_num; j ++)
     {
       if(proc_index[sem_index][j] == -1) continue;
       block_pid = proc_index[sem_index][j];  // record the proc 
       proc_index[sem_index][j] = -1;
       break;
     }
     int i;
     for(i = 0; i < NPROC; i ++)
       if(procs[i].pid == block_pid) break;
     
     // sprint("\n\n!! proc %d no longer wait  %d , sem now is %d !!\n\n", procs[i].pid, sem_index, signal[sem_index] + 1);
